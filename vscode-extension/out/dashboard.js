...
class DashboardPanel {
    ...
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel('adaptiveKDashboard', 'Adaptive-K Dashboard', column, {
            enableScripts: true,
    ...
    _handleCalculation(message) {
        // Handle ROI calculations from webview
        const { tokens, model, savingsRate } = message;
        vscode.window.showInformationMessage(`Calculated savings: ${savingsRate}% for ${tokens} tokens using ${model}`);
    }
    _update() {
    ...
    _getHtmlContent() {
        ...
        const defaultModel = config.get('defaultModel', 'deepseek-v3');
        const savingsRate = config.get('savingsRate', 35);
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">

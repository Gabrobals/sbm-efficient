...
\begin{abstract}
We present \textbf{Adaptive-K routing}, a method that dynamically selects the number of experts in Mixture-of-Experts (MoE) models based on routing entropy. Instead of using a fixed top-$k$ experts per token, our approach uses fewer experts when the router is confident (low entropy) and more experts when uncertain (high entropy). We validate this approach on four production MoE architectures: Mixtral 8x7B (\textbf{31.0\%} compute reduction), Qwen-MoE (\textbf{32.4\%}), OLMoE-1B-7B (\textbf{24.7\%}), and NVIDIA Nemotron 3 Nano (\textbf{33.3\%}, validated January 2026). Furthermore, we demonstrate that Adaptive-K composes multiplicatively with other optimizations (quantization, speculative decoding), achieving up to \textbf{96\%} total compute reduction. Our method is a drop-in replacement for existing MoE routing and requires no model retraining.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Empirical validation} on four production MoE models (Mixtral, Qwen-MoE, OLMoE, Nemotron 3)
...
Nemotron 3 Nano & 128+1 & 6 & 30B total, 3.5B active \\
\bottomrule
\end{tabular}
...
\textbf{Adaptive-K} & \textbf{1.38} & \textbf{69.0\%} & 3.87 & 70.4\% \\
\bottomrule
\end{tabular}
\caption{Mixtral 8x7B results: \textbf{31.0\%} compute reduction with 0.2\% quality impact.}
\label{tab:mixtral}
\end{table}
...
\subsubsection{NVIDIA Nemotron 3 Nano (Validated January 2026)}

We validated Adaptive-K on NVIDIA's Nemotron 3 Nano, a production hybrid Mamba-Transformer MoE with 128 routed experts plus 1 shared expert, using top-6 routing.

\begin{table}[h]
\centering
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{Baseline} & \textbf{Adaptive-K} & \textbf{Notes} \\
\midrule
Average K & 6.0 & 4.0 & 33.3\% reduction \\
Router Entropy & - & 5.23 bits & 74.7\% of max (7.0) \\
Expert Compute & 100\% & 66.7\% & \textbf{33.3\% savings} \\
\bottomrule
\end{tabular}
\caption{Nemotron 3 Nano results: \textbf{33.3\%} compute reduction validated on 128-expert MoE.}
\label{tab:nemotron}
\end{table}

\textbf{Methodology}: Since Nemotron 3 does not support \texttt{output\_router\_logits}, we extracted pre-top-k logits via forward hooks on \texttt{backbone.layers.X.mixer.gate} modules, computing full 128-expert logits as \texttt{hidden\_states @ router\_weight.T}.

\textbf{Key finding}: Router entropy averages 5.23 bits out of 7.0 maximum ($\log_2(128)$), indicating moderate confidence that enables significant K reduction.
...
\bottomrule
\end{tabular}
\caption{Impact of threshold selection on efficiency and quality. Avg K formula: $(\%K{=}1 \times 1) + (\%K{=}2 \times 2)$, e.g., $\theta{=}1.275$: $0.62 \times 1 + 0.38 \times 2 = 1.38$.}
\label{tab:thresholds}
\end{table}

Lower thresholds $\rightarrow$ more aggressive savings but slight quality impact. The optimal threshold $\theta = 1.275$ balances 31\% compute savings with minimal perplexity increase (+0.08).
...
\midrule
Adaptive-K only (31.0\\%) & 31.0\\% & 31.0\\% & -0.2\\% \\
...
\textbf{Key insight}: The 90\\%+ compute reduction represents a $10\\times$ efficiency improvement while maintaining output quality within 1\\% of baseline.
